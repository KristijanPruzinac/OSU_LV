...
KNN_gscv = GridSearchCV(pipe, param_grid, cv=10, scoring='accuracy', return_train_score=False)
...
SVM_model = svm.SVC(kernel='rbf', gamma=10, C=0.1)
SVM_model.fit(X_train_n, y_train)
...
pipe = Pipeline([('scaler', StandardScaler()),
                 ('svc', svm.SVC(kernel='rbf', gamma=1, C=0.1))])
...
SVC_gscv = GridSearchCV(pipe, param_grid, cv=10, scoring='accuracy', return_train_score=False)
...
pipe = Pipeline([('scaler', StandardScaler()),
                 ('svc', svm.SVC(kernel='sigmoid', gamma=1, C=0.1))])
...
pipe = Pipeline([('scaler', StandardScaler()),
                 ('svc', svm.SVC(kernel='linear', gamma=1, C=0.1))])
